...
describe('AuthService', () => {
  ...
  const mockUser = (): User => {
    const user = new User('John', 'Doe', 'john@example.com', 'hashed-password', UserRole.STUDENT);
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: {
            sendNewStudentRegistrationEmail: jest.fn().mockResolvedValue(undefined),
  ...
  describe('validateUser', () => {
    it('should return user data without password when credentials are valid', async () => {
      ...
      expect(em.findOne).toHaveBeenCalledWith(User, { email: 'john@example.com' });
  ...
  describe('login', () => {
    it('should return access_token and user on valid credentials', async () => {
      ...
      const result = await service.login({ email: 'john@example.com', password: 'correct-password' });
  ...
  describe('registerStudent', () => {
    ...
    it('should register a new student and return user without password', async () => {
      ...
      expect(em.findOne).toHaveBeenCalledWith(User, { email: 'jane@example.com' });
    ...
    it('should throw ConflictException if email already exists', async () => {
      ...
      await expect(service.registerStudent(dto)).rejects.toThrow(ConflictException);
    ...
    it('should send registration email after creating the user', async () => {
      ...
      expect(emailService.sendNewStudentRegistrationEmail).toHaveBeenCalledTimes(1);
    });

    it('should not fail registration when email sending fails', async () => {
      em.findOne.mockResolvedValue(null);
      (bcrypt.hash as jest.Mock).mockResolvedValue('hashed');
      emailService.sendNewStudentRegistrationEmail.mockRejectedValue(new Error('SMTP down'));
  ...
  describe('registerTutor', () => {
    ...
    it('should complete tutor registration with valid invitation token', async () => {
      const tutorUser = new User('', '', 'tutor@example.com', '', UserRole.TUTOR);
      ...
      expect(em.findOne).toHaveBeenCalledWith(User, { invitationToken: 'valid-invitation-token' });
    ...
    it('should throw NotFoundException for invalid invitation token', async () => {
      ...
      await expect(service.registerTutor(dto)).rejects.toThrow(NotFoundException);
    });

    it('should throw NotFoundException if user found is not a tutor', async () => {
      const studentUser = new User('', '', 'student@example.com', '', UserRole.STUDENT);
  ...
  describe('forgotPassword', () => {
    it('should set reset token and send email when user exists', async () => {
      ...
      expect(em.flush).toHaveBeenCalled();
      expect(emailService.sendPasswordResetEmail).toHaveBeenCalledWith(user, 'mock-uuid-token');
  ...
  describe('createTutorInvitation', () => {
    it('should create a tutor placeholder and send invitation email', async () => {
      ...
      const result = await service.createTutorInvitation('newtutor@example.com');
      ...
      expect(emailService.sendTutorInvitation).toHaveBeenCalledWith('newtutor@example.com', 'mock-uuid-token');
    ...
    it('should not fail when invitation email sending fails', async () => {
      em.findOne.mockResolvedValue(null);
      emailService.sendTutorInvitation.mockRejectedValue(new Error('SMTP error'));
