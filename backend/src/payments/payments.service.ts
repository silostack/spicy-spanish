...
export class PaymentsService {
  ...
  async createTransaction(createTransactionDto: CreateTransactionDto) {
    ...
    if (!student) {
      throw new NotFoundException(
        `Student with ID ${createTransactionDto.studentId} not found`,
      );
    }
    ...
    if (createTransactionDto.packageId) {
      pkg = await this.packageRepository.findOne({
        id: createTransactionDto.packageId,
      });
      if (!pkg) {
        throw new NotFoundException(
          `Package with ID ${createTransactionDto.packageId} not found`,
        );
      }
    }
  ...
  async createStripeCheckoutSession(checkoutDto: StripeCheckoutDto) {
    ...
    if (!student) {
      throw new NotFoundException(
        `Student with ID ${checkoutDto.studentId} not found`,
      );
    }

    const pkg = await this.packageRepository.findOne({
      id: checkoutDto.packageId,
    });
    if (!pkg) {
      throw new NotFoundException(
        `Package with ID ${checkoutDto.packageId} not found`,
      );
    }
  ...
  async getStudentBalance(studentId: string) {
    ...
    return {
      ...
      availableHours: Math.round((totalHoursPurchased - hoursUsed) * 100) / 100,
    };
  }
  ...
  async getPaymentStats() {
    ...
    const totalPayments = await this.transactionRepository.count({
      status: TransactionStatus.COMPLETED,
    });
